...
int parse_command(int argc, char **argv)
{
  ...
  while (argc > 0)
  {
    ...
    else if (strcmp(argv[0], "debug_bulkload") == 0)
    {
      ...
      assert((start == input->get_key(0)) && (end == input->get_key(keynum - 1)));
    ...
    else if (strcmp(argv[0], "debug_lookup") == 0)
    {
      ...
      assert((start == input->keys[1]) && (end == input->keys[2 * keynum - 1]));
    ...
    else if (strcmp(argv[0], "debug_insert") == 0)
    {
      ...
      { // initiate keys
        ...
        KEY_TYPE start, end;
        the_treep->check(&start, &end);
        assert((start == input->keys[0]) && (end == input->keys[2 * keynum - 1]));
