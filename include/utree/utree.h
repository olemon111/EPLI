...
const uint64_t SPACE_PER_THREAD = 200ULL * 1024ULL * 1024ULL * 1024ULL;
const uint64_t SPACE_OF_MAIN_THREAD = 200ULL * 1024ULL * 1024ULL * 1024ULL;
...
btree::btree()
{
  ...
  list_head = (list_node_t *)NVM::data_alloc->alloc(sizeof(list_node_t));
  printf("list_head=%p\n", list_head);
  list_head->next = NULL;
  height = 1;
}
...
void btree::insert(entry_key_t key, char *right)
{
  ...
  list_node_t *n = (list_node_t *)NVM ::data_alloc->alloc(sizeof(list_node_t));
  // printf("n=%p\n", n);
  n->next = NULL;
  n->key = key;
