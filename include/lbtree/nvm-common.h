...
static inline void NVMFLUSH_STAT_print()
{
 ...
 * @param src   src line
 */
static inline void writeLineMOVNT(char *dest, char *src)
{
  asm volatile(
      "movdqa   %4, %%xmm0    \n\t"
      "movdqa   %5, %%xmm1    \n\t"
...
private:
  char computeCheckSum(char *line)
  {
    ...
    unsigned long long v =
        (p[0] & 0xffffffffffffff00ULL) + p[1] + p[2] + p[3] + p[4] + p[5] + p[6] + p[7];
    v += v >> 32;
    v += v >> 16;
    v += v >> 8;
    return (char)v;
...
public:
  ...
  int readLog(nl_log_pointer_t *pos, char *buf, int len)
  {
    ...
    while ((len > 0) && checkByteOne(pos->nextline_ptr_ - CACHE_LINE_SIZE, pos->version_))
    {
  ...
  int readLogReverse(nl_log_pointer_t *pos, char *buf, int len)
  {
    ...
    while ((len > 0) && checkByteOne(pos->nextline_ptr_, pos->version_))
    {
