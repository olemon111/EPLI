...
struct FTNode {
  ...
  int right_boundary;  // end position (exclusive) in input array that this node
                       // represents
  double left_limit; // these are the key range in this partition 
  double right_limit;
  ...
  int num_keys = 0;
  void *data_node = nullptr; // store the pointer the data node generated by the corresponding fanout tree node
};
...
double compute_level(const std::pair<T, P> values[], int num_keys,
  ...
  for (int i = 0; i < fanout; i++) {
    ...
    boundary_value = ((i + 1) - b) / a;
    //boundary_value = (static_cast<double>(i + 1) / fanout - node->model_.b_) / node->model_.a_;
    //if(!std::is_floating_point<T>::value){
    //  boundary_value = std::ceil(boundary_value);
    //}
    right_boundary =
        i == fanout - 1
            ? num_keys
            : static_cast<int>(
                  std::lower_bound(values, values + num_keys, boundary_value,
    ...
    if (left_boundary == right_boundary) {
      used_fanout_tree_nodes.push_back(
          {level, i, 0, left_boundary, right_boundary, 0, 0, false, 0, 0, 0, 0, 0});
      continue;
    }
    LinearModel<T> model;
...
#ifdef NEW_COST_MODEL
    used_fanout_tree_nodes.push_back(
        {level, i, node_cost, left_boundary, right_boundary, 0, 0, false,
         stats.num_search_cost, stats.num_insert_cost, model.a_, model.b_,
         right_boundary - left_boundary, nullptr});
#else
    used_fanout_tree_nodes.push_back(
        {level, i, node_cost, left_boundary, right_boundary, 0, 0, false,
...
std::pair<int, double> find_best_fanout_bottom_up(
    ...
    int max_data_node_keys, double expected_insert_frac = 0, bool approximate_model_computation = true,
  ...
  // Now the min/max limit is actualy incorrect
  fanout_tree.push_back(
      {{0, 0, best_cost, 0, num_keys,0,0,false, 0, 0, 0, 0, num_keys}});
  for (int fanout = 2, fanout_tree_level = 1; fanout <= max_fanout;
    ...
    //original converge condition of Alex
    ...
    // Add more condition to make the fanout search process more robust
    // Jump out condition will be stricter
    // Only if the data proportion num_keys/total_keys is less than 10%
    // || the best cost is smaller than one threshold (e.g, 100)
    // I cannot use && since maybe in some bottom layer, it is really hard to fit => always has larger than 100 cost
    // For the bottom layer, we really need a way to solve non-uniform distribution problem 
    if ((static_cast<double>(num_keys) / total_keys < 0.1 || best_cost <= 100) && 
        fanout_costs.size() >= 6 &&
        fanout_costs[fanout_costs.size() - 1] >
            fanout_costs[fanout_costs.size() - 2] &&
        fanout_costs[fanout_costs.size() - 2] >
            fanout_costs[fanout_costs.size() - 3]&&
        fanout_costs[fanout_costs.size() - 3] >
            fanout_costs[fanout_costs.size() - 4]&&
        fanout_costs[fanout_costs.size() - 4] >
            fanout_costs[fanout_costs.size() - 5]&&
        fanout_costs[fanout_costs.size() - 5] >
            fanout_costs[fanout_costs.size() - 6]) {
      break;
    }
...
std::pair<int, double> find_best_fanout_top_down(
  ...
  fanout_tree.push_back({{0, 0, overall_cost, 0, num_keys, 0, 0, true}});
      ...
      if (node_split_cost < tree_node.cost) {
        ...
        for (int i = 0; i < 2; i++) {
          ...
          new_level.push_back({fanout_tree_level, 2 * tree_node.node_id + i,
                               node_costs[i], boundaries[i], boundaries[i + 1],0, 0,
                               true, node_stats[i].num_search_cost,
                               node_stats[i].num_insert_cost, node_models[i].a_,
                               node_models[i].b_,
...
int find_best_fanout_existing_node(const AlexModelNode<T, P, Alloc>* parent,
                                   int bucketID, int total_keys,
                                   std::vector<FTNode>& used_fanout_tree_nodes,
                                   int max_fanout, bool build_sorted_node = false) {
  // Repeatedly add levels to the fanout tree until the overall cost of each
  // level starts to increase
  ...
  //double left_boundary_value = node->min_limit_;
 // double right_boundary_value = node->max_limit_;
  double left_boundary_value =
      (start_bucketID - parent->model_.b_) / parent->model_.a_; // left key value
  double right_boundary_value =
      (end_bucketID - parent->model_.b_) / parent->model_.a_; // right key value
  if (left_boundary_value != node->min_limit_)
  {
    printf("the left boundary vlaue is %.25f\n", left_boundary_value);
    printf("the min limit is %.25f\n", node->min_limit_);
    double parent_left_limit = (0 - parent->model_.b_) / parent->model_.a_; 
    printf("the parent left limit is %.25f\n", parent_left_limit);

    std::cout << "The left boundary is wrong!!!" << std::endl;
  }

  if (right_boundary_value != node->max_limit_)
  {
    printf("the right boundary vlaue is %.25f\n", right_boundary_value);
    printf("the min limit is %.25f\n", node->max_limit_);
    double parent_right_limit = (parent->num_children_ - parent->model_.b_) / parent->model_.a_; 
    printf("the parent right limit is %.25f\n", parent_right_limit);
    std::cout << "The right boundary is wrong!!!" << std::endl;
    exit(-1);
  }
...
#ifdef HASH
  if(build_sorted_node) node->build_sorted_slots();
  // first sort the key values based on key value, and then slice it according to fanout
  for (int fanout = 1, fanout_tree_level = 0; fanout <= max_fanout;
    ...
    double right_limit = node->min_limit_;
    for (int i = 0; i < fanout; i++) {
      // BT: slice the boundary equally using the key value.
      left_boundary = right_boundary;
      right_boundary = i == fanout - 1 ? node->num_keys_
                                       : node->lower_bound(((i + 1) - b) / a, true);
      left_limit = right_limit;
      right_limit = i == fanout -1 ? node->max_limit_ :  (((i + 1) - b) / a);

      if (left_boundary == right_boundary) {
        new_level.push_back({fanout_tree_level, i, 0, left_boundary,
                             right_boundary, left_limit, right_limit, false, 0, 0, 0, 0, 0});
        continue;
      }
      int num_actual_keys = 0;
      LinearModel<T> model;
      typename AlexDataNode<T, P>::order_iterator_type it(node, left_boundary, true);
      ...
      new_level.push_back({fanout_tree_level, i, node_cost, left_boundary,
                                right_boundary,left_limit, right_limit, false, stats.num_search_cost,
                                stats.num_insert_cost, model.a_, model.b_,
                                num_actual_keys});
#else
      new_level.push_back({fanout_tree_level, i, node_cost, left_boundary,
                           right_boundary, left_limit, right_limit, false, stats.num_search_iterations,
...
template <class T, class P>
int find_best_fanout_existing_node_without_parent(AlexDataNode<T, P>* node, int total_keys,
                                   std::vector<FTNode>& used_fanout_tree_nodes,
                                   int max_fanout, bool build_sorted_node = false) {
  // Repeatedly add levels to the fanout tree until the overall cost of each
  // level starts to increase
