      ...
      while (true)
      {
        ...
        if (cur == child_node)
        {
          ...
          if (cur_node->children_[ID] == child_node)
          {
            ...
            if (write_lock)
            {
              if (cur_node->get_write_lock())
              { // it could be locked so that it is not deleted
                return true;
              }
            }
            else
  ...
  private:
                         ...
                         data_node_type *left_leaf, data_node_type *right_leaf)
    {
      ...
      do
      {
        ...
        {
          if (prev_leaf == old_leaf->prev_leaf_)
          { // ensure to lock the correct node
    ...
    RETRAVEL:
      ...
      else
      {
        ...
        int end_bucketID =
            start_bucketID + repeats; // first bucket with different child
        if (used_fanout_tree_nodes.empty())
        { // Now only focus on split into two nodes
          assert(fanout_tree_depth == 1);
